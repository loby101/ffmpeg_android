...
int watermarking( JNIEnv* env, jobject thiz , const char* video_path, const char* png_path, const char* out_path)
{
    ...
    //Input YUV
    FILE *fp_in=fopen("/sdcard/rollcap/rollcap_video/ROLLCAP_20170220111138961_1080p.mp4","rb+");
    if(fp_in==NULL){
    ...
    //Output YUV
    FILE *fp_out=fopen("/sdcard/rollcap/rollcap_video/output.yuv","wb+");
    if(fp_out==NULL){
    ...
    snprintf(args, sizeof(args),"video_size=%dx%d:pix_fmt=%d:time_base=%d/%d:pixel_aspect=%d/%d",in_width,in_height,AV_PIX_FMT_YUV420P, 1, 25,1,1);
